...
check_fixed "$policy" '- `DOTFILES_ALLOW_VSCODE_NOPASSWD_ALL=0` (default) removes broad `vscode` sudoers grants and keeps constrained sudoers rules only.' 'policy build-time toggle hardened default'
check_fixed "$policy" '- `DOTFILES_ALLOW_VSCODE_NOPASSWD_ALL=1` installs explicit broad `vscode` debug sudo (`vscode ALL=(ALL) NOPASSWD:ALL`) for temporary local debugging.' 'policy build-time toggle debug override'
