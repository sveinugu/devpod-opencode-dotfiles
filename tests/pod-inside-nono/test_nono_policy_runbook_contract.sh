...
check_fixed "$policy" '- `HUB_ALLOW_VSCODE_SUDO_NOPASSWD_ALL=0` (default) removes broad `vscode` sudoers grants and keeps constrained sudoers rules only.' 'policy build-time toggle hardened default'
check_fixed "$policy" '- `HUB_ALLOW_VSCODE_SUDO_NOPASSWD_ALL=1` installs explicit broad `vscode` debug sudo (`vscode ALL=(ALL) NOPASSWD:ALL`) for temporary local debugging.' 'policy build-time toggle debug override'
...
check_fixed "$policy" 'bash tests/pod-inside-nono/test_nono_profile_layout.sh' 'policy verification profile-layout test'
check_fixed "$policy" 'bash tests/pod-inside-nono/test_opencode_secure_wrapper_contract.sh' 'policy verification wrapper-contract test'
check_fixed "$policy" 'bash tests/pod-inside-nono/test_nono_identity_integration_contract.sh' 'policy verification identity-contract test'
...
check_fixed "$bare_hub" 'devspace build --var HUB_ALLOW_VSCODE_SUDO_NOPASSWD_ALL=1' 'bare-hub notes debug sudo override build command'
...
check_fixed "$lifecycle" 'devspace build --var HUB_ALLOW_VSCODE_SUDO_NOPASSWD_ALL=1' 'lifecycle notes debug sudo override build command'
