...
grep -F 'sudo -n -u "$agent_user" -- "$nono_binary" run --profile "$profile_path" -- /usr/bin/env OPENCODE_CONFIG_CONTENT=' "$wrapper" >/dev/null || fail "wrapper must switch to agent user before entering nono sandbox"
...
grep -F 'NOPASSWD: /home/vscode/.local/bin/nono run --profile * -- /usr/bin/env OPENCODE_CONFIG_CONTENT=* /home/vscode/.opencode/bin/opencode *' "$dockerfile" >/dev/null || fail "Dockerfile must include constrained sudoers rule for pre-sandbox user-switch path"
...
expected_rule = f'NOPASSWD: /home/vscode/.local/bin/nono run --profile * -- /usr/bin/env OPENCODE_CONFIG_CONTENT=* {default_path} *'
