...
require_pod_inside_nono_test 'test_allow_direnv_managed_worktrees_envrc_repair'

script="$repo_root/bin/allow-direnv-managed-worktrees"
new_worktree_script="$repo_root/bin/new-worktree"

fail() {
  printf 'FAIL test_allow_direnv_managed_worktrees_envrc_repair: %s\n' "$1" >&2
  exit 1
}
...
temp_root="$(context_resolve_temp_root_workspace_or_fail 'test_allow_direnv_managed_worktrees_envrc_repair')"
tmpdir="$(context_make_test_tmpdir "$temp_root" 'test_allow_direnv_managed_worktrees_envrc_repair')"
...
target_checkout="$workspace_root/work/feature/missing-envrc"
...
grep -F 'export HUB_DIR=' "$target_checkout/.envrc" >/dev/null || fail 'expected managed envrc content'

printf 'PASS test_allow_direnv_managed_worktrees_envrc_repair\n'
