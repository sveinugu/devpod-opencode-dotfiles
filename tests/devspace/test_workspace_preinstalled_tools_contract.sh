...
grep -F '/home/vscode/.local/bin/nono run --profile * -- /usr/bin/env OPENCODE_CONFIG_CONTENT=' "$dockerfile" >/dev/null || fail "Dockerfile sudoers contract must allow runtime wrapper handoff through nono as agent"
