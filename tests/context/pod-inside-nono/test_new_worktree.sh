...
require_workspace_pod 'test_new_worktree' 'bash tests/context/run.sh pod-inside-nono'
require_inside_nono_sandbox 'test_new_worktree' 'bash tests/context/run.sh pod-inside-nono'
...
[ -f "$workspace_root/state/hub/lanes/registry.tsv" ] || fail "missing hub lane registry"
[ -f "$workspace_root/state/hub/work/feature/top-level/lane-binding.env" ] || fail "missing hub lane binding pointer for feature/top-level"
[ -f "$workspace_root/state/hub/work/feature/top-level-auto/lane-binding.env" ] || fail "missing hub lane binding pointer for feature/top-level-auto"
grep -F 'LANE_ID=feature/top-level' "$workspace_root/state/hub/work/feature/top-level/lane-binding.env" >/dev/null || fail "hub lane pointer should record lane id separately"
grep -F 'REPO_IDENTITY=hub' "$workspace_root/state/hub/work/feature/top-level/lane-binding.env" >/dev/null || fail "hub lane pointer should record repo identity"
grep -F 'STATUS=active' "$workspace_root/state/hub/work/feature/top-level/lane-binding.env" >/dev/null || fail "hub lane pointer should start active"
grep -F 'PARENT_ARTIFACT_ANCHORS=' "$workspace_root/state/hub/work/feature/top-level/lane-binding.env" >/dev/null || fail "hub lane pointer should include explicit parent artifact anchor field"
grep -F 'SESSION_TASK_ID=' "$workspace_root/state/hub/work/feature/top-level/lane-binding.env" >/dev/null || fail "hub lane pointer should include explicit session task field"
grep -F 'SESSION_OWNER=' "$workspace_root/state/hub/work/feature/top-level/lane-binding.env" >/dev/null || fail "hub lane pointer should include explicit session owner field"
grep -F 'ROUTING_STATE=unbound' "$workspace_root/state/hub/work/feature/top-level/lane-binding.env" >/dev/null || fail "hub lane pointer should include explicit routing state"
grep -F $'feature/top-level\thub\tfeature/top-level\t' "$workspace_root/state/hub/lanes/registry.tsv" >/dev/null || fail "hub registry should track feature/top-level binding"
grep -F $'feature/top-level-auto\thub\tfeature/top-level-auto\t' "$workspace_root/state/hub/lanes/registry.tsv" >/dev/null || fail "hub registry should track feature/top-level-auto binding"
...
[ -f "$workspace_root/state/repos/child-source/lanes/registry.tsv" ] || fail "missing child lane registry"
[ -f "$workspace_root/state/repos/child-source/work/feature/child/lane-binding.env" ] || fail "missing child lane binding pointer for feature/child"
[ -f "$workspace_root/state/repos/child-source/work/feature/child-auto/lane-binding.env" ] || fail "missing child lane binding pointer for feature/child-auto"
grep -F 'LANE_ID=feature/child' "$workspace_root/state/repos/child-source/work/feature/child/lane-binding.env" >/dev/null || fail "child lane pointer should record lane id separately"
grep -F 'REPO_IDENTITY=child-source' "$workspace_root/state/repos/child-source/work/feature/child/lane-binding.env" >/dev/null || fail "child lane pointer should record child repo identity"
grep -F 'STATUS=active' "$workspace_root/state/repos/child-source/work/feature/child/lane-binding.env" >/dev/null || fail "child lane pointer should start active"
grep -F 'PARENT_ARTIFACT_ANCHORS=' "$workspace_root/state/repos/child-source/work/feature/child/lane-binding.env" >/dev/null || fail "child lane pointer should include explicit parent artifact anchor field"
grep -F 'SESSION_TASK_ID=' "$workspace_root/state/repos/child-source/work/feature/child/lane-binding.env" >/dev/null || fail "child lane pointer should include explicit session task field"
grep -F 'SESSION_OWNER=' "$workspace_root/state/repos/child-source/work/feature/child/lane-binding.env" >/dev/null || fail "child lane pointer should include explicit session owner field"
grep -F 'ROUTING_STATE=unbound' "$workspace_root/state/repos/child-source/work/feature/child/lane-binding.env" >/dev/null || fail "child lane pointer should include explicit routing state"
grep -F $'feature/child\tchild-source\tfeature/child\t' "$workspace_root/state/repos/child-source/lanes/registry.tsv" >/dev/null || fail "child registry should track feature/child binding"
grep -F $'feature/child-auto\tchild-source\tfeature/child-auto\t' "$workspace_root/state/repos/child-source/lanes/registry.tsv" >/dev/null || fail "child registry should track feature/child-auto binding"
