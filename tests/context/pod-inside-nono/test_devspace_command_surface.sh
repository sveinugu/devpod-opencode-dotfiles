...
require_pod_inside_nono_test 'test_devspace_command_surface'
...
grep -F 'HUB_ALLOW_VSCODE_SUDO_NOPASSWD_ALL:' "$cfg" >/dev/null || fail "devspace.yaml vars should define HUB_ALLOW_VSCODE_SUDO_NOPASSWD_ALL"
...
grep -F 'HUB_PROVISION_ORIGIN_URL="${HUB_PROVISION_ORIGIN_URL:-git@github.com:sveinugu/devpod-opencode-dotfiles.git}"' "$cfg" >/dev/null || fail "provision pipeline should pass HUB_PROVISION_ORIGIN_URL with SSH default into pod provision"
...
grep -F 'HUB_ALLOW_VSCODE_SUDO_NOPASSWD_ALL: ${HUB_ALLOW_VSCODE_SUDO_NOPASSWD_ALL}' "$cfg" >/dev/null || fail "workspace image should consume resolved HUB_ALLOW_VSCODE_SUDO_NOPASSWD_ALL variable"
