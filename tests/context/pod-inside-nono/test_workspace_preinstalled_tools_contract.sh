...
require_workspace_pod 'test_workspace_preinstalled_tools_contract' 'bash tests/context/run.sh pod-inside-nono'
require_inside_nono_sandbox 'test_workspace_preinstalled_tools_contract' 'bash tests/context/run.sh pod-inside-nono'
...
grep -F 'vscode ALL=(root) NOPASSWD: /usr/local/libexec/dotfiles-launch-opencode-nono --setpriv-binary * --nono-binary * --profile * --agent-uid * --agent-gid * --runtime-home * --runtime-xdg-config-home * --runtime-xdg-cache-home * --runtime-xdg-data-home * --runtime-xdg-state-home * --opencode-xdg-state-home * --runtime-path * --opencode-config-content * --raw-opencode-binary * --' "$dockerfile" >/dev/null || fail "Dockerfile sudoers contract must allow runtime wrapper handoff when opencode is invoked without explicit CLI args"
