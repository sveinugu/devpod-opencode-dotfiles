...
./scripts/lib/validate_install_source_tree.sh "$source_root" "$source_root/install.sh" >"$tmpdir/ok.out"
grep -F "ok: validated source path" "$tmpdir/ok.out" >/dev/null

ln -snf /etc/passwd "$source_root/escape"
if ./scripts/lib/validate_install_source_tree.sh "$source_root" "$source_root/escape" >"$tmpdir/escape.out" 2>&1; then
...
printf 'gitdir: /etc\n' > "$source_root/.git"
if ./scripts/lib/validate_install_source_tree.sh "$source_root" "$source_root/install.sh" >"$tmpdir/gitdir.out" 2>&1; then
