...
- Before Task launch for new scoped delegation, validate the packet against the canonical pre-dispatch checks in AGENTS.
- Refuse malformed or interpretive packets instead of silently correcting them.
- If validation fails, stop before launch, do not emit the handoff wording, and do not emit `Session:` / `Resume:` metadata for a launch that did not occur.
...
- Preview any non-trivial packet before dispatch by showing the exact outgoing packet and obtaining explicit user approval.
