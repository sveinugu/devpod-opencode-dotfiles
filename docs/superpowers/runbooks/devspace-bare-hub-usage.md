...
Image-build override note: use `devspace build` for hardened default images and `devspace build --var HUB_ALLOW_VSCODE_SUDO_NOPASSWD_ALL=1` only for temporary broad-`sudo` local debugging. The variable is defined in `devspace.yaml` under `vars` (`source: env`, `default: "0"`), and this repo sets `DEVSPACE_ENV_FILE` to `.env` for project-local overrides.
