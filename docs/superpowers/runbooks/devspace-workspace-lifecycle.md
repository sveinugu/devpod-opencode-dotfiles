...
Default behavior is hardened (`HUB_ALLOW_VSCODE_SUDO_NOPASSWD_ALL=0`), which removes `/etc/sudoers.d/vscode` and keeps only the constrained sudoers rules used by the secure launch chain.
