...
10. **Managed envrc exports:** export exactly `HUB_DIR`, `HUB_MAIN_DIR`, `HUB_STATE_DIR`, `HUB_TMP_DIR`, `DYN_REPO_DIR`, `DYN_REPO_DEFAULT_BRANCH`, `DYN_REPO_DEFAULT_DIR`, `DYN_REPO_STATE_DIR`, `DYN_REPO_TMP_DIR`, `DYN_WORKTREE_DIR`, `DYN_WORKTREE_STATE_DIR`, `DYN_WORKTREE_TMP_DIR`, `TMPDIR`, `TMP`, and `TEMP`; do not export `DYN_REPO_MAIN_DIR`.
...
12. **Managed bare-repo excludes:** use tracked `scripts/lib/bare-excludes.list` as the source of truth for generated-artifact ignore patterns; `scripts/lib/ensure-bare-excludes.sh` must overwrite `<bare>/info/exclude`; top-level `.bare` excludes are reset by bootstrap/provision management paths, child bare repos are seeded on creation only, routine runtime commands do not rewrite excludes, and exclude drift remains warning-only in verification.
13. **User-facing command layout:** use `bin/` for in-pod human commands without `.sh`, `scripts/` for automation entrypoints, `scripts/lib/` for helpers, and `ops/host/` for host-runner sources.
14. **Explicit install-branch override term:** in this plan, an **explicit install-branch override** means a current-run, caller-intended override for branch selection (for example `HUB_INSTALL_BRANCH=<branch> devspace run-pipeline provision` or `repair`, or equivalent direct `install.sh` inputs). Ambient values inherited from managed `.envrc` after sourcing prior `state/hub/etc/install.env` are installed-state context, not an explicit install-branch override.
15. **Installed-branch state contract:** `install.sh` is the authoritative writer of installed-branch state. It must export and persist `HUB_INSTALL_BRANCH` and `HUB_INSTALL_BRANCH_DIR` to `/workspaces/dotfiles/state/hub/etc/install.env`. It must hard-fail when an explicit install-branch override declares a different branch/dir than the checkout it is running from, but it must not treat stale values inherited from prior `/workspaces/dotfiles/state/hub/etc/install.env` state as such an override; a fresh run from another checkout must replace the previous installed-branch state.
16. **Installed-branch worktree policy:** keep `/workspaces/dotfiles/main` attached to `main` only. When an explicit install-branch override selects a non-`main` branch, provision/repair must ensure a matching worktree exists under `/workspaces/dotfiles/work/<branch-name>` and then run that worktree's `install.sh` instead of repointing `main/`. Without an explicit install-branch override, provision defaults to `main`, while repair first consults current installed-branch state from `state/hub/etc/install.env` and uses that non-`main` worktree when it is still valid.
17. **Navigation command surface:** `dhub` is the canonical install-root resolver name, paired with `dre` and `dwt`. `dhub` is implemented as a shell function in `.config/shell/workspace-navigation.zsh` backed by `scripts/lib/resolve-install-target.sh`; no compatibility alias is shipped in v1. Keep resolver behavior explicit and keep the "no implicit fallback" rule for all navigation commands.
18. **Child default-branch fidelity:** child-repo onboarding and related tooling must detect and preserve each child repo's exact remote default branch name instead of normalizing it to `main` or any other fixed branch name.
19. **Top-level primary-branch policy in v1:** top-level provision/repair/doctor/navigation checks are `main`-only for the controlled top-level repo and must fail clearly when `main` is unavailable; no implicit mapping to hub root is allowed.
...
- Create: `../../../tests/pod-inside-nono/test_devspace_command_surface.sh`
- Create: `../../../tests/pod-inside-nono/test_workspace_manifest_contract.sh`
- Create: `../../../tests/host/test_ssh_contract.sh`
...
- Create: `../../../tests/host/test_workspace_provision.sh`
- Create: `../../../tests/host/test_devspace_dev_preflight.sh`
...
- Create: `../../../tests/host/test_devspace_doctor.sh`
- Create: `../../../tests/host/test_workspace_repair.sh`
- Create: `../../../tests/host/test_devspace_destroy.sh`
...
- Modify: `../../../tests/pod-outside-nono/test_create_hub_repo.sh`
- Modify: `../../../tests/pod-inside-nono/test_new_worktree.sh`
- Create: `../../../tests/pod-outside-nono/test_workspace_navigation_commands.sh`
- Create: `../../../tests/pod-outside-nono/test_workspace_navigation_shell.sh`
- Modify: `../../../tests/pod-inside-nono/test_workspace_preinstalled_tools_contract.sh`
...
bash tests/pod-inside-nono/test_workspace_manifest_contract.sh
bash tests/host/test_ssh_contract.sh
...
```bash
kubectl apply --dry-run=client -f k8s/devspace-bare-hub
bash tests/host/test_workspace_provision.sh
bash tests/host/test_devspace_doctor.sh
...
- Create: `k8s/devspace-bare-hub/workspace-pvc.yaml`
- Create: `k8s/devspace-bare-hub/workspace-deployment.yaml`
- Test: `../../../tests/pod-inside-nono/test_devspace_command_surface.sh`
- Test: `../../../tests/pod-inside-nono/test_workspace_manifest_contract.sh`
- Test: `../../../tests/host/test_ssh_contract.sh`

- [ ] **Step 1: Write the failing command-surface test first**

`../../../tests/pod-inside-nono/test_devspace_command_surface.sh` should fail until `devspace.yaml` exists and should assert all of the following exact contracts:
...
`../../../tests/pod-inside-nono/test_workspace_manifest_contract.sh` should parse the YAML and fail until the manifest files exist. It must assert:
...
`../../../tests/host/test_ssh_contract.sh` should start as a failing contract placeholder and must spell out these exact checks for the Task-1/Task-3 SSH path:
...
```bash
git add devspace.yaml k8s/devspace-bare-hub/workspace-pvc.yaml k8s/devspace-bare-hub/workspace-deployment.yaml tests/pod-inside-nono/test_devspace_command_surface.sh tests/pod-inside-nono/test_workspace_manifest_contract.sh tests/host/test_ssh_contract.sh
...
```bash
git add scripts/lib/validate_install_source_tree install.sh .config/opencode/AGENTS.md .config/opencode/agents/maestro.md .config/opencode/agents/senior-implementer.md docs/superpowers/runbooks/devspace-bare-hub-usage.md tests/pod-outside-nono/test_install_validate_source.sh tests/pod-outside-nono/test_install_local_source_contract.sh tests/pod-inside-nono/test_bare_hub_guardrails.sh
...
- Create: `scripts/preflight-devspace-dev.sh`
- Modify: `devspace.yaml`
- Test: `../../../tests/host/test_workspace_provision.sh`
- Test: `../../../tests/host/test_devspace_dev_preflight.sh`
- Test: `../../../tests/host/test_ssh_contract.sh`

- [ ] **Step 1: Write the failing provision tests first**

`../../../tests/host/test_workspace_provision.sh` must cover at least these contracts:
...
`../../../tests/host/test_devspace_dev_preflight.sh` must cover these contracts:
...
```bash
bash tests/host/test_workspace_provision.sh
bash tests/host/test_devspace_dev_preflight.sh
```
...
```bash
git add scripts/lib/hub-repo-core.sh scripts/provision-workspace.sh scripts/preflight-devspace-dev.sh devspace.yaml tests/host/test_workspace_provision.sh tests/host/test_devspace_dev_preflight.sh
...
- Modify: `devspace.yaml`
- Create: `docs/superpowers/runbooks/devspace-workspace-lifecycle.md`
- Test: `../../../tests/host/test_devspace_doctor.sh`
- Test: `../../../tests/host/test_workspace_repair.sh`
- Test: `../../../tests/host/test_devspace_destroy.sh`

- [ ] **Step 1: Write the failing tests first**

`../../../tests/host/test_devspace_doctor.sh` must assert:
...
`../../../tests/host/test_workspace_repair.sh` must assert:
...
`../../../tests/host/test_devspace_destroy.sh` must assert that the host-side pipeline deletes both the Deployment/pod and the PVC.
...
```bash
bash tests/host/test_devspace_doctor.sh
bash tests/host/test_workspace_repair.sh
bash tests/host/test_devspace_destroy.sh
```
...
```bash
git add ops/check-workspace.sh bin/repair-workspace ops/destroy-workspace.sh devspace.yaml docs/superpowers/runbooks/devspace-workspace-lifecycle.md tests/host/test_devspace_doctor.sh tests/host/test_workspace_repair.sh tests/host/test_devspace_destroy.sh
...
- Create: `scripts/lib/worktree-env.sh`
- Modify: `Dockerfile`
- Test: `../../../tests/pod-outside-nono/test_create_hub_repo.sh`
...
`../../../tests/pod-outside-nono/test_create_hub_repo.sh` must assert:
...
`../../../tests/pod-inside-nono/test_new_worktree.sh` must assert:
...
- generated `.envrc` exports exactly `HUB_DIR`, `HUB_MAIN_DIR`, `HUB_STATE_DIR`, `HUB_TMP_DIR`, `DYN_REPO_DIR`, `DYN_REPO_DEFAULT_BRANCH`, `DYN_REPO_DEFAULT_DIR`, `DYN_REPO_STATE_DIR`, `DYN_REPO_TMP_DIR`, `DYN_WORKTREE_DIR`, `DYN_WORKTREE_STATE_DIR`, `DYN_WORKTREE_TMP_DIR`, `TMPDIR`, `TMP`, and `TEMP`, and does not export `DYN_REPO_MAIN_DIR`;
...
```bash
bash tests/pod-outside-nono/test_create_hub_repo.sh
bash tests/pod-inside-nono/test_new_worktree.sh
bash tests/pod-inside-nono/test_workspace_preinstalled_tools_contract.sh
```
...
```bash
git add bin/clone-repo bin/new-worktree scripts/lib/validate_hub_repo_root scripts/lib/worktree-env.sh Dockerfile docs/superpowers/runbooks/devspace-bare-hub-usage.md tests/pod-outside-nono/test_create_hub_repo.sh tests/pod-inside-nono/test_new_worktree.sh tests/pod-inside-nono/test_workspace_preinstalled_tools_contract.sh
...
`../../../tests/pod-outside-nono/test_create_hub_repo.sh` must now assert:
...
`../../../tests/pod-inside-nono/test_new_worktree.sh` must now assert:
...
`../../../tests/pod-outside-nono/test_workspace_navigation_commands.sh` must assert:
...
```bash
git add bin/dre bin/dwt bin/clone-repo bin/new-worktree scripts/lib/hub-repo-core.sh tests/pod-outside-nono/test_create_hub_repo.sh tests/pod-inside-nono/test_new_worktree.sh tests/pod-outside-nono/test_workspace_navigation_commands.sh
...
`../../../tests/pod-outside-nono/test_workspace_navigation_shell.sh` must assert:
...
`../../../tests/pod-inside-nono/test_bare_hub_guardrails.sh` must now assert:
...
```bash
git add install.sh .config/shell/workspace-navigation.zsh docs/superpowers/runbooks/devspace-bare-hub-usage.md docs/superpowers/runbooks/devspace-workspace-lifecycle.md tests/pod-outside-nono/test_workspace_navigation_shell.sh tests/pod-inside-nono/test_bare_hub_guardrails.sh
...
- Modify: `bin/new-worktree`
- Modify: `.config/shell/workspace-navigation.zsh`
- Modify: `../../../tests/host/test_workspace_provision.sh`
- Modify: `../../../tests/host/test_devspace_doctor.sh`
- Modify: `../../../tests/pod-inside-nono/test_new_worktree.sh`
...
```bash
git add scripts/provision-workspace.sh ops/check-workspace.sh bin/new-worktree .config/shell/workspace-navigation.zsh tests/host/test_workspace_provision.sh tests/host/test_devspace_doctor.sh tests/pod-inside-nono/test_new_worktree.sh tests/pod-outside-nono/test_workspace_navigation_commands.sh docs/superpowers/runbooks/devspace-bare-hub-usage.md
