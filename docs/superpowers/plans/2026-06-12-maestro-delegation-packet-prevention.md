...
- Keep runtime/plugin work explicitly deferred in policy text.
...
3. The canonical chapter defines required refusal-before-launch behavior, including no Task call, no fake handoff wording, and no fabricated session metadata.
...
5. The anti-scatter checklist is reordered so validation happens before Task launch and session metadata appears only after successful launch.
6. `.config/opencode/agents/maestro.md` mirrors the operational rules to validate before launch, refuse malformed packets instead of silently fixing them, and preview non-trivial packets, while still treating AGENTS as canonical.
7. The current subagent stop-rule remains substantively unchanged.
8. Docs-only contract tests pass after the documentation update and no active contradictory packet-policy test remains.
...
## Task 1: Update the canonical prevention policy in AGENTS.md
...
## Task 2: Mirror Maestro-only operational behavior in `maestro.md`
...
## Task 3: Refresh docs-only verification and resolve stale drift checks
...
- [ ] Update the canonical contract test so it checks pre-dispatch prohibition, refusal-before-launch semantics, preview gating for non-trivial packets, and the reordered anti-scatter checklist.
