...
**Architecture:** Keep this slice test-only and minimal. Do not edit either runbook; change only the two stale assertions in `tests/context/pod-inside-nono/test_bare_hub_guardrails.sh` so the host-side `ssh` and `verify-ssh` checks point at `docs/superpowers/runbooks/devspace-workspace-lifecycle.md`, while every other assertion remains unchanged.
...
- Remaining baseline failure: `tests/context/pod-inside-nono/test_bare_hub_guardrails.sh`
...
- Re-run `tests/context/pod-inside-nono/test_bare_hub_guardrails.sh` before and after the change.

### Out of scope

- No runbook edits.
- No changes to any other assertion in `tests/context/pod-inside-nono/test_bare_hub_guardrails.sh`.
...
- Modify: `tests/context/pod-inside-nono/test_bare_hub_guardrails.sh` — repoint the two stale SSH/lifecycle assertions to the lifecycle runbook.
...
- Verify only: `bash tests/context/pod-inside-nono/test_bare_hub_guardrails.sh`
...
Update only these two assertions in `tests/context/pod-inside-nono/test_bare_hub_guardrails.sh`:
...
```bash
git add tests/context/pod-inside-nono/test_bare_hub_guardrails.sh
git commit -m "test(docs): fix hc7 bare hub guardrails contract"
```

## User Check-in

If `bash tests/context/pod-inside-nono/test_bare_hub_guardrails.sh` still fails after the two-line update, stop and ask whether the remaining failure is still in scope for this minimal HC-7 slice before touching any additional assertions or runbooks.
